...
// This file defines a wrapper for replacing solo carriage return characters (\r)
// with carriage-return + line feed (\r\n)
//
// Copied from github.com/qri-io/dataset/dsio/replace_cr.go
...
// ReplaceReader wraps an io.Reader. on every call of Read. it looks for
...
// Use with caution.
func ReplaceReader(data io.Reader) io.Reader {
	return crlfReplaceReader{
		rdr: bufio.NewReader(data),
	}
}

// ReplaceReaderWithSize instaties a reader with a given buffer size
func ReplaceReaderWithSize(data io.Reader, size int) io.Reader {
	return crlfReplaceReader{
